...
class ErrorCode(Enum):
    ...
    PARAMS_MISMATCH  = 'Parameters arity mismatch'
...
class ProcedureCall(AST):
    def __init__(self, proc_name, actual_params, token):
        self.proc_name = proc_name
        self.actual_params = actual_params # list of VarDecl nodes.
        self.token = token
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_ProcedureCall(self, node):
        # 1. buscar el símbolo del procedure en la tabla de símbolos.
        # 2. obtener el número de parámetros almacenados en la tabla de símbolos.
        # 3. comparar el número de parámetros contra el número actual.
        proc_symbol = self.current_scope.lookup(node.proc_name.value)
        total_params = len(proc_symbol.params)
        if total_params != len(node.actual_params):
            raise SemanticError(
                error_code=ErrorCode.PARAMS_MISMATCH,
                token=node.token,
                message='Current params does not match declared ones',
            )
...
class Interpreter(NodeVisitor):
    ...
    def visit_Var(self, node):
        var_name = node.value
        # obtener el AR desde la pila de llamadas.
        ar = self.call_stack.peek()
